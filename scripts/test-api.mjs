...
execSync(`npx tsc --outDir ${outDir} --module commonjs --moduleResolution node \
  ...
  lib/adminAuth.ts lib/db.ts lib/enrichUtils.ts \
...
// tsc preserves src layout: lib/ files land in outDir/lib/, api/ files in outDir/api/
// Use createRequire so CJS exports.default unwraps correctly from ESM scope.
const req = createRequire(import.meta.url);
const { verifyAdminCode } = req(join(outDir, 'lib/adminAuth.js'));
const adminHandler = req(join(outDir, 'api/admin/verify.js')).default;
const giftsHandler = req(join(outDir, 'api/gifts.js')).default;
const createGiftHandler = req(join(outDir, 'api/admin/gifts.js')).default;
const giftByIdHandler = req(join(outDir, 'api/admin/gifts/[id].js')).default;
const markPurchasedHandler = req(join(outDir, 'api/mark-purchased.js')).default;
